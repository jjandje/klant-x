...
@php
    $breadcrumbs = [
        ...
        __('account.articles') => route('profile.articles'),
        $article->title => false,
    ];
@endphp
...
<section class="articles-detail">
    <div class="row">
        <div class="col-lg-5 order-1 order-lg-0 mt-5 mt-lg-0">
            <div class="articles-detail__article-info d-flex flex-row align-items-center">
                @if($article->getAuthorName())
                <div class="articles-detail__author d-flex flex-row align-items-center">
                    ...
                    <p>{{ $article->getAuthorName() }}</p>
                </div>
                @endif
                @if($article->getGoalNames())
                <div class="articles-detail__goal d-flex flex-row align-items-center">
        ...
        <div class="offset-lg-1 col-lg-6 order-0 order-lg-1">

            <div class="recipes-detail__actions d-flex flex-row align-items-center">
                <div class="recipes-detail__link d-flex flex-row align-items-center{{ backpack_user()->favoriteArticles->contains($article->id) ? ' active' : '' }}" id="addArticleToFavorites" data-uid="{{ backpack_user()->id }}" data-aid="{{ $article->id }}">
            ...
            <div class="articles-detail__image">
...
@section('after_scripts')
    <script type="text/javascript">
        ...
        function ajaxCall(user_id, article_id) {
            $.ajax({
                type: 'POST',
                url: '/platform/blogs/addtofavorite',
                data: {uid: user_id, aid: article_id},
                success: function(data) {
                    if(data.success) {
                        $('#addArticleToFavorites').addClass('active');
                        new Noty({
                            type: 'success',
                            text: data.message,
                            timeout: 2000,
                        ...
                        $('#addArticleToFavorites').removeClass('active');
                        new Noty({
                            type: 'success',
                            text: data.message,
                            timeout: 2000,
        ...
        $('#addArticleToFavorites').on('click', function(e) {
            e.preventDefault();
            var $this = $(this),
                user_id = $this.attr('data-uid'),
                article_id = $this.attr('data-aid');
            ajaxCall(user_id, article_id);
        });
